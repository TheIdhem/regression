...
def validate_answer(maze, path):
    ...
    for cell in path:
        temp = []
        for i in maze.get_neighbors(cell):
            temp.append(i[0])
        neighbor_set = set(temp)
...
class Maze:
    ...
    def check_wall(self, cell, ncell):
        ...
        if cell[0] - ncell[0] == 1:  # cell is right of ncell
            return [self.get_cell(ncell)[0], "right"]
        elif cell[0] - ncell[0] == -1:  # cell is left of ncell
            return [self.get_cell(cell)[0], "left"]
        elif cell[1] - ncell[1] == 1:  # cell is under ncell
            return [self.get_cell(ncell)[1], "under"]
        elif cell[1] - ncell[1] == -1:  # cell is top of ncell
            return [self.get_cell(cell)[1], "top"]
    ...
    def get_neighbors(self, cell):
        ...
        return [[x, self.check_wall(cell, x)[1]] for x in around if self.cell_is_valid(x) and self.check_wall(cell, x)[0] == 0]
...
if __name__ == "__main__":
    ...
    dfs_path = Q_Learning(m)
    try:
        validate_answer(m, dfs_path)
        print(("Q Learning solved maze. cost: ", len(dfs_path), "cells visited"))

        print("Display Q Learning solution? [y/n]")
        display_command = input()
        if "y" in display_command:
            d.draw_path(dfs_path)
    except AssertionError as e:
        print(("Q Learning answer is invalid: " + e.message))
